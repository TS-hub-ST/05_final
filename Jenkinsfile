pipeline {
    ...
    parameters {
        ...
        string(
            name: 'API_HOST',
            defaultValue: 'http://192.168.56.1:3002',
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        timeout 60 bash -c 'until curl -f http://localhost:3002/health; do sleep 2; done' || exit 1

                        # Check attractions endpoint
                        curl -f http://localhost:3002/movies || exit 1
